...
int unitlogdCloseBootLog()
{
    ...
    if (UNITLOGD_BOOT_LOG_FILE) {
        ...
        if ((rv = fclose(UNITLOGD_BOOT_LOG_FILE)) != 0) {
            logError(CONSOLE, "src/unitlogd/file/file.c", "unitlogdCloseBootLog", errno,
...
int unitlogdOpenLog(const char *mode)
{
    if (!UNITLOGD_LOG_FILE) {
        ...
        if (!UNITLOGD_LOG_FILE) {
            logError(CONSOLE | UNITLOGD_BOOT_LOG, "src/unitlogd/file/file.c", "unitlogdOpenLog",
                     errno, strerror(errno), "Unable to open the file '%s' in mode '%s'", logPath,
                     mode);
            return -1;
        }
    }
...
int unitlogdCloseLog()
{
    ...
    if (UNITLOGD_LOG_FILE) {
        ...
        if ((rv = fclose(UNITLOGD_LOG_FILE)) != 0) {
            logError(CONSOLE | UNITLOGD_BOOT_LOG, "src/unitlogd/file/file.c", "unitlogdCloseLog",
                     errno, strerror(errno), "Unable to close the file '%s'", logPath);
        }
...
int unitlogdOpenIndex(const char *mode)
{
    if (!UNITLOGD_INDEX_FILE) {
        ...
        if (!UNITLOGD_INDEX_FILE) {
            logError(CONSOLE | UNITLOGD_BOOT_LOG, "src/unitlogd/file/file.c", "unitlogdOpenIndex",
                     errno, strerror(errno), "Unable to open the file '%s' in mode '%s'", logPath,
                     mode);
            return -1;
        }
...
int unitlogdCloseIndex()
{
    ...
    if (UNITLOGD_INDEX_FILE) {
        ...
        if ((rv = fclose(UNITLOGD_INDEX_FILE)) != 0) {
            logError(CONSOLE | UNITLOGD_BOOT_LOG, "src/unitlogd/file/file.c", "unitlogdCloseIndex",
                     errno, strerror(errno), "Unable to close the file '%s'", logPath);
        }
...
int unitlogdOpenKmsg(const char *mode)
{
    if (!UNITLOGD_KMSG_FILE) {
        ...
        if (!UNITLOGD_KMSG_FILE) {
            logError(CONSOLE | UNITLOGD_BOOT_LOG, "src/unitlogd/file/file.c", "unitlogdOpenKmsg",
                     errno, strerror(errno), "Unable to open the file '%s' in mode '%s'", kmsgPath,
                     mode);
            return -1;
        }
    }
...
int unitlogdCloseKmsg()
{
    ...
    if (UNITLOGD_KMSG_FILE) {
        ...
        if ((rv = fclose(UNITLOGD_KMSG_FILE)) != 0) {
            logError(CONSOLE | UNITLOGD_BOOT_LOG, "src/unitlogd/file/file.c", "unitlogdCloseKmsg",
                     errno, strerror(errno), "Unable to close the file '%s'", kmsgPath);
        }
...
char *getLogOffset()
{
    ...
    if (fseeko(UNITLOGD_LOG_FILE, 0, SEEK_END) == -1) {
        logError(UNITLOGD_BOOT_LOG, "src/unitlogd/file/file.c", "getLogOffset", errno,
