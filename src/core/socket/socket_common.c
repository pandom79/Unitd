...
void sockMessageInRelease(SockMessageIn **sockMessageIn)
{
    if (*sockMessageIn) {
...
int initSocket(struct sockaddr_un *name)
{
...
int unitdSockConn(int *socketConnection, struct sockaddr_un *name)
{
    int rv = 0;
    if ((rv = connect(*socketConnection, (const struct sockaddr *)name,
                      sizeof(struct sockaddr_un))) == -1) {
        logErrorStr(SYSTEM, !USER_INSTANCE ? "Unitd system instance could be not running!" :
...
int readMessage(int *socketFd, char **buffer, int *bufferSize)
{
    int rv = 0;
    ...
    while (1) {
        if ((rv = uRecv(*socketFd, *buffer, *bufferSize, MSG_PEEK | MSG_TRUNC)) == -1) {
            logError(CONSOLE, "src/core/socket/socket_common.c", "readMessage", errno,
                     strerror(errno), "Recv error");
            goto out;
        }
        assert(rv > 0);
...
int sortUnitsByName(const void *unitDisplayA, const void *unitDisplayB)
{
    return strcasecmp((*(Unit **)unitDisplayA)->name, (*(Unit **)unitDisplayB)->name);
}

void setValueForBuffer(char **buffer, int value)
{
    assert(*buffer);
    if (value == -1 || value == -2)
...
Array *getScriptParams(const char *unitName, const char *stateStr, const char *symlOperation,
                       const char *unitPath)
{
...
int sendWallMsg(Command command)
{
    int rv = 0;
    char *msg = NULL;
...
void fillUnitsDisplayList(Array **units, Array **unitsDisplay, ListFilter listFilter)
{
    Unit *unit = NULL;
    bool add = false;
...
int loadAndCheckUnit(Array **unitsDisplay, bool isAggregate, const char *unitName, bool parse,
                     Array **errors)
{
    ...
    if (!USER_INSTANCE) {
        loadUnits(unitsDisplay, UNITS_PATH, NULL, NO_STATE, isAggregate, unitName,
                  PARSE_SOCK_RESPONSE, parse);
    } else {
        loadUnits(unitsDisplay, UNITS_USER_PATH, NULL, NO_STATE, isAggregate, unitName,
                  PARSE_SOCK_RESPONSE, parse);
        if (*lenUnitsDisplay == oldLen) {
            loadUnits(unitsDisplay, UNITS_USER_LOCAL_PATH, NULL, NO_STATE, isAggregate, unitName,
                      PARSE_SOCK_RESPONSE, parse);
        }
    }
    if (*lenUnitsDisplay == oldLen) {
...
ListFilter getListFilterByCommand(Command command)
{
...
ListFilter getListFilterByOpt(Array *options)
{
...
void applyListFilter(Array **unitsDisplay, ListFilter listFilter)
{
    ...
    for (int i = 0; i < *len; i++) {
        ...
        case STARTED_FILTER:
            match =
                ((pData->pStateData->pState == RUNNING || pData->pStateData->pState == EXITED) &&
