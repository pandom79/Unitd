...
void *startProcess(void *arg)
{
    ...
    Unit *unit = NULL, *unitDep = NULL, *unitConflict = NULL;
    Array *units = NULL, *requires = NULL, *conflicts = NULL, *wantedBy = NULL;
    int lenDeps = 0, lenConflicts = 0, statusThread = 0, *finalStatus, *finalStatusDep,
        *rvThread = NULL, rv = 0;
    ProcessData *pData = NULL, *pDataDep = NULL, *pDataConflict = NULL;
    pthread_mutex_t *unitMutex = NULL, *unitDepMutex = NULL;
    char **cmdline = NULL;
    const char *command = NULL, *unitName = NULL, *unitNameDep = NULL, *unitNameconflict = NULL,
               *desc;
    ...
    if ((rv = pthread_mutex_lock(unitMutex)) != 0) {
        *finalStatus = FINAL_STATUS_FAILURE;
        logError(ALL, "src/core/processes/process.c", "startProcess", rv, strerror(rv),
                 "Unable to acquire the lock of the mutex for the %s unit", unitName);
        goto out;
    }
    if (DEBUG)
            ...
            if (*finalStatusDep == FINAL_STATUS_READY) {
                ...
                if ((rv = pthread_mutex_lock(unitDepMutex)) != 0) {
                    *finalStatusDep = FINAL_STATUS_FAILURE;
                    logError(ALL, "src/core/processes/process.c", "startProcess", rv, strerror(rv),
                             "Unable to lock the mutex for the %s dependency", unitNameDep);
                } else {
                        ...
                        if ((rv = pthread_cond_wait(unitDep->cv, unitDepMutex)) != 0) {
                            *finalStatusDep = FINAL_STATUS_FAILURE;
                            logError(ALL, "src/core/processes/process.c", "startProcess", rv,
                                     strerror(rv),
                                     "Unable to wait for condition variable for the %s dependency",
                                     unitNameDep);
                            break;
                        }
                        if (DEBUG)
                    ...
                    if ((rv = pthread_mutex_unlock(unitDepMutex)) != 0) {
                        *finalStatusDep = FINAL_STATUS_FAILURE;
                        logError(ALL, "src/core/processes/process.c", "startProcess", rv,
                                 strerror(rv), "Unable to unlock the mutex for the %s dependency",
                                 unitNameDep);
                    }
...
out:
    ...
    switch (unit->type) {
    case DAEMON:
        if (*pData->pid != -1 && *pData->exitCode == -1 && pData->pStateData->pState == RUNNING)
    ...
    if ((rv = pthread_cond_broadcast(unit->cv)) != 0) {
        *finalStatus = FINAL_STATUS_FAILURE;
        logError(ALL, "src/core/processes/process.c", "startProcess", rv, strerror(rv),
                 "Unable to send the broadcast signal for the %s unit", unitName);
    ...
    if ((rv = pthread_mutex_unlock(unitMutex)) != 0) {
        *finalStatus = FINAL_STATUS_FAILURE;
        logError(ALL, "src/core/processes/process.c", "startProcess", rv, strerror(rv),
                 "Unable to unlock the mutex for the %s unit", unitName);
        kill(UNITD_PID, SIGTERM);
    }
    /* show the result */
    if (unit->showResult) {
...
int startProcesses(Array **units, Unit *singleUnit)
{
    int rv = 0, result = 0, numThreads = 0, *rvThread;
    Unit *unit = NULL;
...
Array *getRunningUnits(Array **units)
{
...
void *stopProcess(void *arg)
{
    ...
    case TIMER:
        ...
        if ((rv = pthread_mutex_unlock(unitMutex)) != 0) {
            *finalStatus = FINAL_STATUS_FAILURE;
            logError(CONSOLE, "src/core/processes/process.c", "stopProcess", rv, strerror(rv),
                     "Unable to unlock of the mutex for the %s unit (timer case)", unitName);
            goto out;
        }
        ...
        if ((rv = pthread_mutex_lock(unitMutex)) != 0) {
            *finalStatus = FINAL_STATUS_FAILURE;
            logError(CONSOLE, "src/core/processes/process.c", "stopProcess", rv, strerror(rv),
                     "Unable to acquire the lock of the mutex for the %s unit (timer case)",
                     unitName);
            goto out;
        }
        *pData->pStateData = PSTATE_DATA_ITEMS[DEAD];
    ...
    if ((rv = pthread_mutex_unlock(unitMutex)) != 0) {
        *finalStatus = FINAL_STATUS_FAILURE;
        logError(CONSOLE, "src/core/processes/process.c", "stopProcess", rv, strerror(rv),
                 "Unable to unlock of the mutex for the %s unit", unitName);
        goto out;
    }
...
int stopProcesses(Array **units, Unit *unitArg)
{
    int rv = 0, numThreads = 0, *rvThread;
    Unit *unit = NULL;
    const char *unitName = NULL;
...
{
    Unit *unit = NULL;
    int rv = 0, input = 0, restartMax, *restartNum, rvMutex = 0;
    bool restart = false;
    ProcessData **pData = NULL;
    ...
    if ((rv = pthread_mutex_lock(unitPipe->mutex)) != 0) {
        logError(CONSOLE | SYSTEM, "src/core/processes/process.c", "listenPipe", rv, strerror(rv),
                 "Unable to lock the pipe mutex for the %s unit", unitName);
        kill(UNITD_PID, SIGTERM);
    }
    if ((rv = pthread_mutex_unlock(unit->mutex)) != 0) {
        logError(CONSOLE | SYSTEM, "src/core/processes/process.c", "listenPipe", rv, strerror(rv),
                 "Unable to lock the mutex for the %s unit", unitName);
        kill(UNITD_PID, SIGTERM);
    }
    while (1) {
        if ((rv = uRead(unitPipe->fds[0], &input, sizeof(int))) == -1) {
            logError(CONSOLE | SYSTEM, "src/core/processes/process.c", "listenPipe", errno,
                     strerror(errno), "Unable to read from pipe for '%s'", unitName);
            kill(UNITD_PID, SIGTERM);
            goto out;
        }
        ...
        */
        ...
        if (failureCmd) {
            ...
            cmdlineRelease(cmdline);
            if (rv == 0)
                logSuccess(SYSTEM, "%s: '%s' failure command executed successfully!", unitName,
                           failureCmd);
            else {
                logErrorStr(SYSTEM, "%s: '%s' failure command returned %d exit code!", unitName,
                            failureCmd, rv);
                arrayAdd(unit->errors, getMsg(-1, UNITD_ERRORS_ITEMS[UNITD_GENERIC_ERR].desc));
            }
        }
        if ((restartMax > 0 && *restartNum < restartMax) || (restartMax == -1 && restart)) {
...
Array *getRestartableUnits(Array **units)
{
...
int listenPipes(Array **units, Unit *unitArg)
{
    int rv = 0, numThreads = 0;
    Unit *unit = NULL;
    const char *unitName = NULL;
        ...
        for (int i = 0; i < numThreads; i++) {
            ...
            if ((rv = pthread_create(&threads[i], &attr[i], listenPipe, unit)) != 0) {
                logError(CONSOLE | SYSTEM, "src/core/processes/process.c", "listenPipes", rv,
                         strerror(rv),
                         "Unable to create the detached thread for '%s' (listen pipe)", unitName);
                kill(UNITD_PID, SIGTERM);
            } else {
                if (DEBUG)
                    logInfo(UNITD_BOOT_LOG,
...
void *closePipe(void *arg)
{
    Unit *unit = NULL;
    int rv = 0, *rvThread, output;
    const char *unitName = NULL;
    Pipe *unitPipe = NULL;
    ...
    if (unit->pipe) {
        ...
        if ((rv = uWrite(unitPipe->fds[1], &output, sizeof(int))) == -1) {
            logError(CONSOLE | SYSTEM, "src/core/processes/process.c", "closePipe", errno,
                     strerror(errno), "Unable to write into pipe for the %s unit", unitName);
            kill(UNITD_PID, SIGTERM);
        }
        if ((rv = pthread_mutex_lock(mutex)) != 0) {
            logError(CONSOLE | SYSTEM, "src/core/processes/process.c", "closePipe", rv,
                     strerror(rv), "Unable to acquire the lock of the pipe mutex for the %s unit",
                     unitName);
            kill(UNITD_PID, SIGTERM);
        }
...
int closePipes(Array **units, Unit *unitArg)
{
    int rv = 0, numThreads = 0, *rvThread;
    Unit *unit = NULL;
    const char *unitName = NULL;
