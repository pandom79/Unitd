...
2 = Not a symlink
*/
int readSymLink(const char *symLink, char **wherePoints)
{
    struct stat sb;
...
void addEnvVar(Array **envVarArr, const char *envVarName, const char *envVarValue)
{
    char *envVar = NULL;
...
State getStateByStr(char *stateArg)
{
    State ret = NO_STATE;
...
int getDefaultStateStr(char **destDefStateSyml)
{
    int rv = 0;
...
int setNewDefaultStateSyml(State newDefaultState, Array **messages, Array **errors)
{
    int rv = 0;
    ...
    if (rv != 0) {
        ...
        arrayAdd(*messages, getMsg(-1, UNITS_MESSAGES_ITEMS[UNIT_CREATED_SYML_MSG].desc, to, from));
...
void arrayPrint(int options, Array **array, bool hasStrings)
{
...
bool isKexecLoaded()
{
    ...
    if ((fp = fopen("/sys/kernel/kexec_loaded", "r")) == NULL) {
        logError(CONSOLE | SYSTEM, "src/core/common/common.c", "isKexecLoaded", errno,
                 strerror(errno), "Unable to open '/sys/kernel/kexec_loaded' file!");
        return res;
    }
...
int writeWtmp(bool isBooting)
{
    int fd, rv = 0;

    if ((fd = open(OUR_WTMP_FILE, O_WRONLY | O_APPEND)) < 0) {
        rv = errno;
        logError(CONSOLE | SYSTEM, "src/core/common/common.c", "writeWtmp", rv, strerror(rv),
                 "Unable to open '%s' file descriptor!", OUR_WTMP_FILE);
        return rv;
    }
    struct utmp utmp = { 0 };
    ...
    if (uWrite(fd, (char *)&utmp, sizeof(utmp)) == -1) {
        rv = errno;
        logError(CONSOLE | SYSTEM, "src/core/common/common.c", "writeWtmp", rv, strerror(rv),
                 "Unable to write into '%s' file!!", OUR_WTMP_FILE);
        return rv;
    }
    if (close(fd) == -1) {
        rv = errno;
        logError(CONSOLE | SYSTEM, "src/core/common/common.c", "writeWtmp", rv, strerror(rv),
                 "Unable to close '%s' file descriptor!", OUR_WTMP_FILE);
        return rv;
    }
    if (isBooting) {
        if ((fd = open(OUR_UTMP_FILE, O_WRONLY | O_APPEND)) < 0) {
            rv = errno;
            logError(CONSOLE | SYSTEM, "src/core/common/common.c", "writeWtmp", rv, strerror(rv),
                     "Unable to open '%s' file descriptor!", OUR_UTMP_FILE);
            return rv;
        }
        if (uWrite(fd, (char *)&utmp, sizeof utmp) == -1) {
            rv = errno;
            logError(CONSOLE | SYSTEM, "src/core/common/common.c", "writeWtmp", rv, strerror(rv),
                     "Unable to write into '%s' file!", OUR_UTMP_FILE);
            return rv;
        }
        if (close(fd) == -1) {
            rv = errno;
            logError(CONSOLE | SYSTEM, "src/core/common/common.c", "writeWtmp", rv, strerror(rv),
                     "Unable to close '%s' file descriptor!", OUR_UTMP_FILE);
            return rv;
        }
    }
...
int unitdUserCheck(const char *userIdStr, const char *userName)
{
    int rv = 0;
    char euirun[5] = { 0 };
    ...
    if (rv != 0) {
        ...
        if (rv == EUIRUN) {
            logErrorStr(CONSOLE | SYSTEM, "Unitd instance is already running for %s user!",
                        userName);
            printf("\n");
        } else {
            logError(CONSOLE | SYSTEM, "src/core/common/common.c", "unitdUserCheck", rv,
...
int setUserData(int userId, struct passwd **userInfo)
{
    int rv = 0;
    ...
    *userInfo = getpwuid(userId);
    ...
    if ((rv = chdir(userHome)) == -1) {
        logError(CONSOLE | SYSTEM, "src/core/common/common.c", "setUserData", errno,
                 strerror(errno), "Chdir (user instance) for %d userId returned -1 exit code",
                 userId);
        rv = 1;
        goto out;
    }
...
void *handleMutexThread(void *arg)
{
    ...
    if (mutexThreadData->lock) {
        if ((rv = pthread_mutex_lock(mutexThreadData->mutex)) != 0) {
            logError(SYSTEM, "src/core/common/common.c", "handleMutexThread", rv, strerror(rv),
                     "Unable to acquire the lock of the mutex");
...
int handleMutex(pthread_mutex_t *mutex, bool lock)
{
    ...
    if ((rv = pthread_create(&thread, NULL, handleMutexThread, &mutextThreadData)) != 0) {
        logError(SYSTEM | CONSOLE, "src/core/common/common.c", "handleMutex", rv, strerror(rv),
                 "Unable to create the thread for the mutex");
        kill(UNITD_PID, SIGTERM);
    }
    if ((rv = pthread_join(thread, (void **)&rvThread)) != 0) {
        logError(SYSTEM | CONSOLE, "src/core/common/common.c", "handleMutex", rv, strerror(rv),
                 "Unable to join the thread for the mutex");
        kill(UNITD_PID, SIGTERM);
    }
    if (*rvThread != 0)
        rv = *rvThread;
...
int getMaxFileDesc(int *fdA, int *fdB)
{
    int max = *fdA + 1;
    if (*fdA < *fdB)
        max = *fdB + 1;
...
char *getUnitNameByOther(const char *fromUnitName, PType unitType)
{
    char *unitName = NULL;
    int endIndex = -1;
...
char *getOtherNameByUnitName(const char *unitName, PType pType)
{
    char *otherName = NULL;
    int endIndex = -1;
...
int uRead(int fd, void *buf, size_t nbytes)
{
    int rv = 0;
...
int uWrite(int fd, void *buf, size_t nbytes)
{
    int rv = 0;
...
ssize_t uSend(int fd, const void *buf, size_t nbytes, int flags)
{
    ssize_t rv = 0;
...
ssize_t uRecv(int fd, void *buf, size_t nbytes, int flags)
{
    ssize_t rv = 0;
