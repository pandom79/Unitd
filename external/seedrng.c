...
enum blake2s_lengths { BLAKE2S_BLOCK_LEN = 64, BLAKE2S_HASH_LEN = 32, BLAKE2S_KEY_LEN = 32 };
...
static void blake2s_compress(struct blake2s_state *state, const uint8_t *block, size_t nblocks,
                             const uint32_t inc)
{
    uint32_t m[16];
    uint32_t v[16];
    ...
    *is_creditable = false;
    ...
    if (ret == (ssize_t)len) {
        *is_creditable = true;
        ...
        struct pollfd random_fd = { .fd = open("/dev/random", O_RDONLY), .events = POLLIN };
        if (random_fd.fd < 0)
            return -errno;
...
static bool skip_credit(void)
{
    ...
    return skip && (!strcmp(skip, "1") || !strcasecmp(skip, "true") || !strcasecmp(skip, "yes") ||
                    !strcasecmp(skip, "y"));
}
